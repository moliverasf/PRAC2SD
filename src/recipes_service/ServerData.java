...
public class ServerData {
	...
	public synchronized void removeRecipe(String recipeTitle){
		...
		Recipe rec = this.recipes.get(recipeTitle);
		Operation op = new RemoveOperation(recipeTitle, rec.getTimestamp(), ts);
		
		this.log.add(op);
		this.summary.updateTimestamp(ts);
		this.recipes.remove(recipeTitle);
	}
	
	
	public synchronized void execOperation(AddOperation addOper) {
		
		if (this.log.add(addOper)) {
			
			this.recipes.add(addOper.getRecipe());
		}
	}
	
	public synchronized void execOperation1(RemoveOperation remOper) {
		
		if (this.log.add(remOper)) {
			
			this.recipes.remove(remOper.getRecipeTitle());
		}
